...
class MLP(Block):
    ...
    def __init__(self, in_features, num_classes, hidden_features=(),
        ...
        # ====== YOUR CODE: ======
        in_dims = [in_features] + hidden_features[:-1]
        out_dims = hidden_features
        ...
        blocks.append(Linear(out_dim,num_classes))
        # ========================
...
class ConvClassifier(nn.Module):
    ...
    def _make_feature_extractor(self):
        ...
        N = len(self.filters) ; P = self.pool_every
        ...
        for i in range(N):
            layers.append(nn.Conv2d(in_channels,self.filters[i],3,padding=1))
    ...
    def _make_classifier(self):
        ...
        # ====== YOUR CODE: ======
        '''Calculate Dimensions Robustly'''
        for i in range(N):
            if i%P == 1:
                in_w /= 2
                in_h /= 2
        in_dims = self.filters[-1] * int(in_h * in_w)

        for i in range(M):
            layers.append(nn.Linear(in_dims, self.hidden_dims[i]))
            layers.append(nn.ReLU())
            in_dims = self.hidden_dims[i]
        layers.append(nn.Linear(in_dims, self.out_classes))
        # ========================
        seq = nn.Sequential(*layers)
        return seq
